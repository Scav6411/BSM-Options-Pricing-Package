...
def put_option_price(S, E, tau, r, sigma, D=0):
    D1 = d1(S, E, tau, r, sigma, D=0)
    D2 = d2(S, E, tau, r, sigma, D=0)
...
def option_pricing(csv, csv_file_path = None ,input_data=None):
    if csv:
        ...
        if input_data is not None:
            ...
            # Check if expiry date is ahead of current date
            if T < t:
                raise ValueError("Expiry date cannot be before the current date.")
            
            tau = (T - t).days / 365.0
            r = r/100
            sigma = sigma/100
            
            d1_val = d1(S, E, tau, r, sigma, D=0)
            d2_val = d2(S, E, tau, r, sigma, D=0)

            if option_type.upper() in ['CALL', 'C']:
                call_price = call_option_price(S, E, tau, r, sigma, D=0)
                return {"Call Option Value": call_price}
            elif option_type.upper() in ['PUT', 'P']:
                put_price = put_option_price(S, E, tau, r, sigma, D=0)
        ...
        # dataset = dataset_handling.data_preprocessing()

# print(dataset)

# print(option_pricing(csv=False, input_data=('Put',199.36,498.22,'03-05-2023','01-03-2023',50.82549851,60.37673209)))
